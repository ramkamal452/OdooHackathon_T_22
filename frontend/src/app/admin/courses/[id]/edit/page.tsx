...
export default function AdminEditCoursePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setHeader({ title: 'Edit Course', subtitle: c.title });
      try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/courses/${courseId}`); toast('Link copied!', 'success'); }}>
          Share
        </Button>
        ...
        <Button variant="outline" size="sm" asChild>
          <Link href="/admin/content-manager"><Layers className="mr-1.5 h-4 w-4" />Content Manager</Link>
        </Button>
      </div>
                    ...
                    <div className="flex flex-wrap gap-1 rounded-md border bg-background p-1">
                      {[
                        ...
                        <Button
                          ...
                          title={btn.title}
                          onClick={() => {
                            const ta = document.getElementById('admin-desc-editor') as HTMLTextAreaElement;
                            if (!ta) return;
